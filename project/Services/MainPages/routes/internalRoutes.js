...
const internalRoutes = [
    ...
    new Router("POST", "/luca-app/main/searchByName", async(req,res) =>{
        try {
            const cookies = req.headers.cookie;
            const tokenStatus = await getTokenStatus(cookies)
            if (!tokenStatus.valid){
                ...
                const controller = new feedController();
                return await controller.searchBarName(req, res);
            }
        } catch (error) {
            console.error(error);
            res.writeHead(500, {'Content-Type': 'text/plain'});
    ...
    },"Get token from full_name",
    "Get an access token after an actor full name.")
];
